...
class Computation():
    ...
    def boundaryExtraction(self, map_data, position):
        ...
        #sometimes inner contour in undetectable
        outter_b = countour_hierarchy_list[0]
        outter_b_inx = outter_b[0]
        if(len(outter_b)>1): 
        ...
        tmpout = np.array(contours[outter_b_inx].copy()).reshape(contours[outter_b_inx].shape[0],contours[outter_b_inx].shape[2])
        ...
        if (len(countour_hierarchy_list)>2):
            ...
            for boundary_inx in inner_b:
                parent_inx = hierarchy[0][boundary_inx][3]
                grandparent_inx = hierarchy[0][parent_inx][3]

                if((outter_b_inx == parent_inx)or (outter_b_inx == grandparent_inx)): 
                    #add only if choldren of outter boundary
...
if __name__=='__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if (not computation.map_data_tmp == None):
            ...
            # TRY COMPUTATION
            if haspos:
                #FOR DEBUG ONLY: uncomment this to see any errors
            ...
            if not failed_comp:
                ...
                # plt.plot(xl_py,yl_py)
                # plt.plot(xl_py[is_bou_free], yl_py[is_bou_free], 'r*')
                # plt.show()
            else: 
