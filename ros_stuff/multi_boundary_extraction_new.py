...
class MergingComputation(Computation):
    ...
    def checkMerge(self):
        ...
        # GET ROBOT POSITION
        try:
            (trans,rot) = listener.lookupTransform(self.tf_merged_map_frame, self.tf_robot_frame, rospy.Time(0))
            #pos in image pixels
        ...
        # for i,rob_pos in enumerate(other_robot_pos):
        ...
        for robot_ns in self.other_robots_ns:
            try:
                (trans,rot) = listener.lookupTransform(self.tf_merged_map_frame, robot_ns+'/base_footprint', rospy.Time(0))
                #pos in image pixels
...
if __name__=='__main__':
    ...
    mc = MergingComputation('tb3_0', ['tb3_1'])
